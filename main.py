...
class GUI(object):
    ...
    def starthighscore(self):
        ...
        textutil.drawtextcentered(self.menusurf, (size[0] // 2, size[1] - 10), self.font, "press enter to continue",
                                  alignment=(1, 2))
        """
    ...
    def startdie(self):
        ...
        self.player.position = [self.world_size[i] // 2 for i in range(2)]
        ...
        for i in range(75):
            ...
            if self.easy:
                speed = 4
            self.addrandom(image=self.astroid_image, groups=(self.astroids, self.objects), rotates=False,
                           speed=random.random() * speed + 5)
        for i in range(300):
            self.addrandom(coin.Coin, self.coin_img, None, random.random() * 12, value=10, rotates=False,
                           groups=(self.objects, self.special_coins, self.coins), friction=0, timeout=-1)
    ...
    def draw(self):
        ...
        if self.player:
            self.offset = [-self.player.position[i] + self.screen.get_size()[i] // 2 for i in range(2)]
            for i in range(2):
                if self.offset[i] < -self.world_size[i] + size[i]:
                    self.offset[i] = -self.world_size[i] + size[i]
                elif self.offset[i] > 0:
                    self.offset[i] = 0
        first = tuple(self.offset[i] % 128 - 128 for i in range(2))
        for x in range(11):
            for y in range(6):
                if (self.state) == "dead":
        ...
        if self.trans:
            ...
            self.screen.blit(self.transsurface, (size[0] // 2 + self.transamount, 0),
                             (size[0] // 2, 0, size[0] // 2, size[1]))
        ...
        if not self.trans:
            ...
            for i in self.objects:
                if (self.state != "dead" or i not in self.enemies):
                    i.update();
            if self.state != "dead":
                for en, bul in pygame.sprite.groupcollide(self.enemies, self.bullets, False, False,
                                                          pygame.sprite.collide_circle).items():
                    ...
                    if killen and bul[0].shooter is not en:
                        ...
                        if hasattr(self, "offset"):
                            pos = tuple(en.position[i] + self.offset[i] for i in range(2))

                            if (all(pos[i] > -32 and pos[i] < self.screen.get_size()[i] + 32 for i in range(2))):
                                self.play_explode()
            ...
            if self.player:
                if self.state == "game" and not self.player_immune:
                    for i in pygame.sprite.spritecollide(self.player, self.bullets, False,
                    ...
                    for i in pygame.sprite.spritecollide(self.player, self.astroids, False,
                                                         pygame.sprite.collide_circle):
                        self.state = "game"
                        self.revive()
    ...
    def addrandommonster(self, ID=-1):
        ...
        if ID == 0 or c == "normal":
            ...
            return self.addrandom(cls=enemy.RotateCellerator, image=self.glowring_img, speed=0, accel=1.5,
                                  friction=0.1, rotates=False,
                                  numcoins=14, ID=1)
        elif ID == 2 or c == "shooter":
            return self.addrandom(cls=enemy.Shooter, image=self.redguy_image, speed=0, accel=1.25, friction=0.1,
                                  rotates=True, radius=17,
                                  numcoins=7, ID=2)
        elif ID == 3 or c == "turtle":
            return self.addrandom(cls=enemy.Turtle, image=self.turtle_image, radius=96, speed=2,
                                  numcoins=28, ID=3)
        elif ID == 4 or c == "orange_rect":
            return self.addrandom(cls=enemy.Orange_Rect, image=self.orange_square_image, speed=2, radius=48,
                                  otherimage=self.white_circle_image,
                                  numcoins=32, rotates=False, ID=4)
        else:
            raise ValueError("c=" + str(c) + ", ID=" + str(ID))

    def addrandom(self, cls=enemy.Enemy, image=None, angle=NotImplemented, speed=5, groups=None,
